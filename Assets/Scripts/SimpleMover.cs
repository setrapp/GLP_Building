...
public class SimpleMover : MonoBehaviour {
	...
	void Update() {
		...
		if (movingTo)
		{
			if ((targetPosition - transform.position).sqrMagnitude < velocity.sqrMagnitude)
			{
	...
	public void MoveTo(Vector3 position, bool overTime = true)
	{
		if (overTime)
		{
			targetPosition = position;
			movingTo = true;
			velocity = (targetPosition - transform.position).normalized * maxSpeed;
		}
		else
		{
